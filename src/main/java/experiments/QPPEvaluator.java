...
public class QPPEvaluator {
    ...
    public Evaluator executeDummy(List<MsMarcoQuery> queries, Similarity sim,
    ...
    public double[] getQPPEstimates(
        ...
        for (MsMarcoQuery query : queries) {
            ...
            TopDocs topDocs = topDocsMap.get(query.getId());
            if (topDocs==null) {
                System.err.println(String.format("No Topdocs found for query %s", query.getId()));
                estimatedScore = 0;
            }
            else
                estimatedScore = qppMethod.computeSpecificity(query, rr, topDocs, qppTopK);
    ...
    public void relativeSystemRanksAcrossMetrics(List<MsMarcoQuery> queries) throws Exception {
    ...
    public void relativeSystemRanksAcrossMetrics(List<MsMarcoQuery> queries, int cutoff) throws Exception {
        Similarity[] sims = modelsToTest();
        for (Similarity sim: sims) {
     ...
     */
    public void relativeSystemRanksAcrossMetrics(Similarity sim, List<MsMarcoQuery> queries, int cutoff) throws Exception {
        int i, j, k;
    ...
    private RegParameters fit(
            List<MsMarcoQuery> trainQueries,
    ...
    public void relativeSystemRanksAcrossMetrics(Similarity sim,
             List<MsMarcoQuery> trainQueries,
             List<MsMarcoQuery> testQueries, int cutoff) throws Exception {
        int i, j, k;
    ...
    different retrieval models. For stability, these numbers should be high.
     */
    public void relativeSystemRanksAcrossSims(Metric m, List<MsMarcoQuery> queries) throws Exception {
        int i, j, k;
        int cutoff = Settings.getNumWanted();
        Similarity[] sims = corrAcrossModels();
        QPPMethod[] qppMethods = qppMethods();
    ...
    public void evaluateQPPAllWithCutoffs(List<MsMarcoQuery> queries) throws Exception {
        QPPMethod[] qppMethods = qppMethods();
        for (QPPMethod qppMethod: qppMethods) {
    ...
    public void evaluateQPPAllWithCutoffs(QPPMethod qppMethod, List<MsMarcoQuery> queries) throws Exception {
    ...
    public void evaluateQPPAtCutoff(QPPMethod qppMethod, List<MsMarcoQuery> queries, int cutoff) throws Exception {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            List<MsMarcoQuery> queries = qppEvaluator.constructQueries();
            ...
            List<MsMarcoQuery> trainQueries = queries.subList(0, splitIndex);
            List<MsMarcoQuery> testQueries = queries.subList(splitIndex, queries.size());
